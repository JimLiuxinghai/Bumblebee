...
const logger = require('../util/lib/log');
/**
 * request()(req, {}).then();
 *
 * 后端接口
...
module.exports = function (req, res) {
    return (options) => {
        ...
        // 设置cookie
        let token = options.token || req.session.token;
        // if (!token) {
        ...
        if (options.api === 'user') {
            ...
            url = `${url}${suffix}api_key=${ENV_CONFIG.ucenter['ai'].apikey}`;
        }
        //TODO:兼容phptoken
        if (options.api === 'php') {
            let suffix = '?';
        ...
        let j = request.jar();
        if (token) {
            let cookie = request.cookie('BWTOKEN=' + token);
        ...
        // jsonp 判断
        if (options.isJsonp) {
            options.data.callback = options.data.callback || 'node';
            options.headers = options.headers || {};
            let cookie = options.headers.Cookie || req.cookies;
            if (!cookie.PHPSESSID) {
                cookie.PHPSESSID = token || req.headers.token || req.headers.phpsessid;
            }
            options.headers.Cookie = util.cookie.dealCookies(cookie);
        } else if (options.isCookie) {
            options.headers.Cookie = util.cookie.dealCookies(req.cookies);
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            request(param, (err, request, body) => {
                ...
                if (!options.isJsonp) {
                    ...
                    // 用户中心 成功code不一致
                    if ((options.api === 'user' || options.api === 'quality') && data.status.code == 10000) {
                    ...
                    if (method === 'GET') {
                        returnData = util.object.parseJsonp(options.data.callback, data);
                    } else {
                        let srt = data.replace(/^\(/, '').replace(/\)$/, '');
                        returnData = JSON.parse(srt);
                    }
                }
                resolve(returnData);
                // util.log.recordLog(logger, {
